...
def transform(original, url):
    ...
    restored=staged.replace(mono_resource,'').replace(default_after,default_before)
    restored=re.sub(r'(?<=[=,])(\s*)Pornhub(?=\s*(?:,|$))',r'\1成人站点',restored,flags=re.M)
    assert restored==original, 'Unexpected non-icon edit'
    return merge(staged.replace('resource-tag-regex=^Kuromis$','resource-tag-regex=^(Kuromis|mono)$'))
...
def main():
    ...
    a.out.chmod(0o600)
    print('PASS: other content preserved; 41 own-host icons; Pornhub rename; mono subscription added disabled; source filters support both providers. No secrets logged.')
